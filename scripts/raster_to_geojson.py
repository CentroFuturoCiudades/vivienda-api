...
from utils.utils import RASTERS_DIR, GEOJSONS_DIR, YEARS, HEX_ID, HEX_RESOLUTION
...
def disjunct_rasters(paths_rasters):
  ...
  disjuncted_rasters = [x.copy(data=np.where(x.values > THRESHOLD, 1, 0)) for x in disjuncted_rasters]
...
def convert_to_geojson(paths_rasters: [str], years: [str]):
...
def as_h3(gdf: GeoDataFrame) -> GeoDataFrame:
  gdf[HEX_ID] = gdf.apply(lambda row: h3.geo_to_h3(row.geometry.y, row.geometry.x, HEX_RESOLUTION), axis=1)
  ...
  h3_df['year'] = h3_df['year'].apply(lambda row: {k: v / max_counts[k] for k, v in row.items()})
...
if __name__ == '__main__':
  ...
  for city in args.cities:
    raster_files = sorted([os.path.join(RASTERS_DIR, city, f'{x}.tif') for x in YEARS])
