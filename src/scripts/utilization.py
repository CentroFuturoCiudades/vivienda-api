...
if __name__ == "__main__":
    ...
    important_block_columns = ["vivtot", "prom_ocup", "pobtot", "block_area", "cvegeo"]
    gdf_lots = pd.merge(gdf_lots.drop(columns=[
                        "block_area"]), gdf_blocks[important_block_columns], on="cvegeo", how="left")
    ...
    gdf_edges = gdf_edges[gdf_edges["name"].isin(
        ZONING_REGULATIONS["streets"])]
    buffer = 0.00015
    ...
    gdf_lots[gdf_lots['street_names'].apply(len) > 0].plot(
        ax=ax, column='zoning', legend=True)
    plt.show()
    ...
    gdf_lots["building_area"] = gdf_lots.apply(
        lambda x: x["building_area"] if x["building_ratio"] > 0.1 else 0, axis=1)
    gdf_lots["building_ratio"] = gdf_lots.apply(
        lambda x: x["building_ratio"] if x["building_ratio"] > 0.1 else 0, axis=1)

    gdf_lots["max_cos"] = gdf_lots.apply(
        lambda x: get_zone_info(x, "COS"), axis=1)
    gdf_lots["max_building_area"] = gdf_lots["lot_area"] * \
        gdf_lots["max_cos"]  # m2
    gdf_lots["max_cus"] = gdf_lots.apply(
        lambda x: get_zone_info(x, "CUS"), axis=1)
    gdf_lots["max_builtup_area"] = gdf_lots["lot_area"] * \
        gdf_lots["max_cus"]  # m2
    ...
    gdf_lots['builtup_area'] = gdf_lots['num_levels'] * \
        gdf_lots['building_area']
    ...
    gdf_lots['home_units'] = gdf_lots['vivtot'] / \
        gdf_lots['cvegeo'].map(counts)
    ...
    gdf_lots["occupancy"] = gdf_lots["population"] + gdf_lots["num_workers"]
    ...
    gdf_lots = gdf_lots.drop(columns=important_block_columns)
    gdf_lots.to_file(
